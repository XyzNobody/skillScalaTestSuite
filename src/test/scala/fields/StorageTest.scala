...
class StorageTest extends StorageTestBase[SkillState]("fields") {
  override def getMainObject = StorageTest
  
  override def create = SkillState.create
  override def createElements(σ: SkillState, n: Int) = for (i ← 0 until n) σ.Fields(i, i)
  ...
  test("Randomized storage test")
  {
    ...
    SingleValueResult.saveGraph("results/storageTest/fields.tex", "Fields test", "axis", "only marks",
