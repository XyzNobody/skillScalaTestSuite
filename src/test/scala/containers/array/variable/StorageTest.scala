package containers.array.variable

import common.StorageTestBase
import common.randomHelpers.UniformDistribution
import common.storage._
import containers.array.variable.api.SkillState
...
class StorageTest extends StorageTestBase[SkillState]("array.variable") {
  ...
  override def createElements(σ: SkillState, n: Int) =
    for (i ← 0 until n) {
      val data = σ.makeAnnotationVarArray()
      for (j ← 0 until 10)
        data += null
      σ.Variable(data)
    }
  ...
  def createMoreVarElements(σ: SkillState, n: Int) =
    for (obj ← σ.Variable.all) {
      val data = obj.variable
      for (j ← 0 until n)
        data += null
    }
  ...
  test("Randomized storage test 1")
  {
    ...
    SingleValueResult.saveGraph("results/storageTest/variable1.tex", "Variable size array test", "axis", "only marks",
  ...
  test("Randomized storage test 2")
  {
    ...
    SingleValueResult.saveGraph("results/storageTest/variable2.tex", "Variable size array test", "axis", "only marks",
        Seq(createRes, writeRes, readRes, createMoreRes, appendRes))
  }
}
